                    ...
                    case 3:
                        if(!(unavailablePosition.includes(positionId))){
                            if(energy() > activeCost*1000){
                                document.getElementById("modeOutput").innerHTML ="剩余能量: "+ (energy(activeCost)*0.001).toFixed(3).slice(0, -2);
                                if (parent) {
...
function chooseAbility(index) {
    if (ready) {
        alert("技能只能在准备之前选择，请取消准备!");
        return;
    }
    switch (index) {
        case 1:
            if(!confirm('是否选择 "移动"(默认6能量消耗): 任意移动一个棋子')){index = null};
            break;
        case 2:
            if(!confirm('是否选择 "陷阱"(默认4能量消耗): 占据一个位置，若敌方在此落子将其化为持续6秒的"障碍"且不计入连子计算')){index = null};
            break;
        case 3:
            if(!confirm('是否选择 "障碍"(默认4能量消耗): 占据一个位置持续6秒，但不计入连子计算')){index = null};
            break;
        default:
            break;
    }
...
function nextTurn() {
    ...
    document.getElementById("nowTurn").innerHTML = "当前回合: "+turn+" ";
    // waiting time
    if (timer) {
        clearInterval(timer);
    }
...
function countDown() {
    ...
    waitingTime--;
    document.getElementById("modeOutput").innerHTML = "剩余时间: "+ waitingTime;
}
