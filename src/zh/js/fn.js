...
function tryConnect(object, id, ifJump, ifAskForMediaStream){
    ...
    switch (object) {
                                ...
                                if (new_Conferee[1] === peer.id) {
                                    ...
                                    if (myIcon) {
                                        ...
                                        while (i < conferee_Nodes.length) {
                                            conferee_Nodes[i].send([5,3, my_Conferee_Index, null, myIcon])
                                            i++;
                                        }
                                    }
                                    let i = 0;
                ...
                guest.on('open', () => {
                    for(let i=0; i<guests.length; i++){
                        ...
                        switch (nodesMap[3]) {  // room type
                            case 0:
                                if (ifJump === 2) {
                                    window.open("./P2PLiveAudience.html?id="+ guest.peer +"&name="+ getMyName());
                                } else {
                            ...
                            case 1:
                                if (ifJump === 2) {
                                    window.open("./P2PGameFiveOnLinePlayer.html?id="+ guest.peer +"&name="+ getMyName());
                                } else {
        ...
        case 2:
            ...
            conferee.on('open', () => {
                if (myIcon) {
                    conferee.send([5,3, my_Conferee_Index, null, myIcon]);
                }
                    ...
                    case 2:
                        ...
                        case 3:
                            if (data[3]) {      // stream
                                ...
                                document.getElementsByClassName('confereeVideos')[data[2]].src = data[3];
                            } else if (data[4]){    // img
                                document.getElementsByClassName('conferees')[data[2]].style.background = data[4];
                            }
                            break;
                    default:
                        break;
        ...
        case 4:     // for Index
            ...
            setTimeout(function() {
                if (root) {    
                    if (!root.open) {
                        document.getElementById("modifyNetWorkName").innerHTML="状态: 连接中...(时间过长, 换个节点?)"
                    }
                }
            }, 3000);
    ...
    if(file){
        if(file.size <= 1048576){
            ...
            reader.onloadend = () => {
                ...
                document.getElementById("uploadImgButton").style.backgroundColor = 'var(--theme-active-color)';
            }
        } else {
            alert("不能超过1MB");
        }
...
function upload_Conferee_Video() {
    if ( document.getElementById('uploadConfereeVideo').checked && conference_Stream) {
        if (my_Conferee_Index) {
            ...
            let i = 0;
            while (i < conferee_Nodes.length) {
                conferee_Nodes[i].send([5,3, my_Conferee_Index, my_Conferee_Stream_Id, null]);
...
function change_Theme(index, theme_color, font_color, ui_color, input_color, active_color, visited_color) {
// get
...
//     document.documentElement.style.setProperty('name', value);
    ...
    font_color = font_color || 'rgb(0,0,0)';
